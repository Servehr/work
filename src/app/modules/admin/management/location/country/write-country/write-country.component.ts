...
export const countryNameRequired = (control: AbstractControl): ValidationErrors | null => 
{
   return control.value?.length === 0 || control.value === null ? { roleNameRequired : 'roleNameRequired' } :  null
}

@Component({
  selector: 'app-write-country',
  standalone: true,
  imports: [RouterModule, ReactiveFormsModule, BotinComponent, InputFieldComponent, ModalComponent, TextAreaComponent],
  templateUrl: './write-country.component.html',
  styleUrl: './write-country.component.scss'
})
export class WriteCountryComponent implements OnInit {
   ...
   constructor()
   {
      this.countryForm = new FormGroup(
        {
          countryName: new FormControl('', [countryNameRequired]),
        }
      ) 
